...
export const updateMap = (map: any, key: string, onChange: Function) => {
  setTimeout(() => {
    generateDataUrl(map, key, onChange);
  }, 500);
};

const generateDataUrl = (map: any, key: string, onChange: Function) => {
  ...
  if (exportContext) {
    ...
    if (currentState.dataUrl !== data) {
      ...
      if (onChange) {
        onChange(newState.dataUrl);
      }
      else {
        // propPanel.defaultValue = newState.dataUrl;
